...
function search() {
   let listElementChilder = document.querySelector('#towns').children;
   let inputSearchElement = document.querySelector('#searchText');
   let resultElement = document.querySelector('#result');

   let foundItems = Array.from(listElementChilder)
      .filter(v => v.textContent.includes(inputSearchElement.value))
      .map(v => {
         v.style.fontWeight = 'bold';
         v.style.textDecoration = 'underline';
         return v;
   ...
   resultElement.textContent = `${foundItemsCount} matches found`
